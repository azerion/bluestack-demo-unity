...
public class SafeAreaHandler : MonoBehaviour
{
...
#if UNITY_EDITOR
            float marginTop = (margin / 2560f) * Screen.height; // ref: 1440x2560
            // Debug.Log("adSpace: " + marginTop + "margin: " + margin);
#else
            float marginTop = margin;
#endif
            safeArea = new Rect(safeArea.x, safeArea.y, safeArea.width,
                safeArea.height - marginTop);
        }
    ...
    void ApplySafeArea(Rect r)
    {
        ...
        if (Logging)
        {
            ...
            float marginTop = (margin / 2960f) * Screen.height; // ref: 1440x2960
            // Debug.Log("margin: " + margin + ", marginTop: " + marginTop);
        }
    }
}